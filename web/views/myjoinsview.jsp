...
<html lang="en" class="no-js">
    <head>
        ...
        <script type="text/javascript">
            function accept(reqId, compId) {
                $.ajax({
                    type: "POST",
                    url: "main?id=<%= Keys.ViewID.MY_JOINS_VIEW_ID %>",
    ...
    <body>
        <div class="divHeader mertoFont"style="height: 130px;margin-top: 1px;">
            ...
            <a href="main?id=<%= Keys.ViewID.MY_JOINS_VIEW_ID%>"><div class="headerItem "><img src="images/header/street_view.png"/><h2>requests</h2></div></a>
            <div class="headerUserinfo">
                <%
                    User user = (User) request.getSession().getAttribute(Keys.SessionAttribute.USER);
                    ArrayList<JoinRequest> joins = (ArrayList<JoinRequest>) request.getAttribute(Keys.AttributeNames.JOINS_ATTRIBUTE_NAME);
                    int compConter = 1;
        ...
        <div class="container">
            <header class="clearfix">
                <h1 style="color:#47a3da;">Join Requests</h1>
            </header>
            ...
            <div class="main">
                <ul class="cbp_tmtimeline">
                    ...
                    <li id='i<%= compConter %>'>
                        <% String[] parts = join.getDate().toString().split(" ");%>
                        <time class="cbp_tmtime" datetime="2013-04-10 18:30"><span><%= parts[0]%></span> <span><%= parts[1]%></span></time>
                        <div class="cbp_tmicon cbp_tmicon-mail"></div>
                        <div class="cbp_tmlabel">
                            <% User owner = User.selectByUserId(DefaultDatabase.getInstance().getConnection(), join.getSender());%>
                            <h4><%= owner.getFirstName() + " " + owner.getLastName()%> </h4>
                            <h2><%= join.getProjectName()%></h2>
                            <p><%= join.getMsg()%></p>
                            <button type="button" onclick="accept(<%= join.getRequestId() %>,'i<%= compConter %>');"> Accept </button>
                            <button type="button" onclick="reject(<%= join.getRequestId() %>,'i<%= compConter %>');"> Reject </button>
                            <% compConter++; %>
                        </div>
                    </li>
                    <% }%>
